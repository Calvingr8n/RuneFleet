...
namespace RuneFleet.Services
{
    public static class EnvReader
    {
        public static Dictionary<string, string> ReadEnvironmentVariablesFromProcess(int pid)
        {
            ...
            using var hProcess = OpenProcess(ProcessAccessFlags.QueryInformation | ProcessAccessFlags.VirtualMemoryRead, false, pid);
            if (hProcess.IsInvalid)
                throw new Win32Exception(Marshal.GetLastWin32Error(), "Failed to open process.");

            bool isWow64;
            if (!IsWow64Process(hProcess.DangerousGetHandle(), out isWow64))
        ...
        private static Dictionary<string, string> ReadEnvironment64(SafeProcessHandle hProcess)
        {
            ...
            int returnLength;
            int status = NtQueryInformationProcess(hProcess.DangerousGetHandle(), 0, ref pbi, Marshal.SizeOf<PROCESS_BASIC_INFORMATION64>(), out returnLength);
            if (status != 0)
            ...
            if (!ReadProcessMemory(hProcess, environmentAddress, buffer, buffer.Length, out _))
        ...
        private static Dictionary<string, string> ReadEnvironment32(SafeProcessHandle hProcess)
        {
            ...
            int returnLength;
            int status = NtQueryInformationProcess32(hProcess.DangerousGetHandle(), 0, ref pbi, Marshal.SizeOf<PROCESS_BASIC_INFORMATION32>(), out returnLength);
            if (status != 0)
                throw new Win32Exception(status, "Failed to query process information.");
        ...
        private static extern SafeProcessHandle OpenProcess(ProcessAccessFlags access, bool inheritHandle, int processId);

        [DllImport("kernel32.dll", SetLastError = true)]
        private static extern bool ReadProcessMemory(SafeProcessHandle hProcess, IntPtr lpBaseAddress, [Out] byte[] lpBuffer, int dwSize, out IntPtr lpNumberOfBytesRead);

        [DllImport("ntdll.dll")]
        private static extern int NtQueryInformationProcess(IntPtr processHandle, int processInformationClass, ref PROCESS_BASIC_INFORMATION64 pbi, int cb, out int returnLength);

        [DllImport("ntdll.dll")]
        private static extern int NtQueryInformationProcess32(IntPtr processHandle, int processInformationClass, ref PROCESS_BASIC_INFORMATION32 pbi, int cb, out int returnLength);
    ...
    // string sessionId = env.ContainsKey("JX_SESSION_ID") ? env["JX_SESSION_ID"] : null; // etc.
}
